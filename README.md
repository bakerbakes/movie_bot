...
A Discord bot for looking up movies, actors, and getting genre-based recommendations via [The Movie Database (TMDb)](https://www.themoviedb.org/) API. Also includes `/moviephone`, which joins your voice channel and plays a custom greeting clip, and `/trivia`, a "guess the movie from the plot" mini-game.
...
- `/trivia` — guess the movie from its plot summary; click one of four buttons before the 30-second timer runs out
...
3. Create a `.env` file in the project root:
   ...
   - `TMDB_REGION` controls which country's streaming/rent/buy providers show up in `/movie`. Optional — defaults to `US` if omitted. Use a [two-letter ISO 3166-1 country code](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) (e.g. `GB`, `CA`, `AU`).
...
| `TMDB_API_KEY` | your TMDb API key | Movie/actor/recommend/trivia commands |
| `TMDB_REGION` | e.g. `US` | (Optional) region for streaming/rent/buy provider data in `/movie`. Defaults to `US`. |
...
- `movie_cog.py` — all command implementations (`/movie`, `/actor`, `/recommend`, `/moviephone`, `/trivia`)
...
### Trivia (`/trivia`)
Pulls a random page from TMDb's `popular` movies list, picks one movie as the correct answer plus three random decoys, and posts the plot summary with four button choices. Wrong guesses turn red and lock out that button (no spam-clicking), but the round stays open to everyone until someone gets it right or the 30-second timer expires. On timeout, the correct answer is revealed automatically.
