...
class MovieCog(commands.Cog):
    ...
    async def trivia(self, interaction: discord.Interaction):
        ...
        # Pull from a wider, less-recency-biased pool: mix popular + top_rated
        source_endpoint = random.choice(["/movie/popular", "/movie/top_rated"])
        page = random.randint(1, 40)
        data = await self._get(source_endpoint, {"page": page})
        ...
        # Get the correct movie's genres so we can pick same-genre decoys
        correct_details = await self._get(f"/movie/{correct['id']}")
        correct_genre_ids = set(correct_details.get("genre_ids", correct.get("genre_ids", []))) if correct_details else set(correct.get("genre_ids", []))
        ...
        # Prefer same-genre decoys; fall back to whatever's available if not enough
        ...
        # Trim the overview so it's a guess, not a giveaway
